...
class CDC(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=0, dilation=0, groups=1,
                 bias=False, theta=1.0):
...
class CDCMasked(nn.Module):

    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=0, dilation=0, groups=1,
                 bias=False, theta=1.0, mask=(0, 2, 6, 8)):
        super(CDCMasked, self).__init__()

        self.theta = theta
        self.kernel_size = kernel_size
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.mask = torch.unsqueeze(torch.arange(in_channels)[:, None] * out_channels +
                                    torch.arange(out_channels)[None, :], -1) + torch.tensor(mask, dtype=torch.int64)
        self.conv_weight = torch.nn.Parameter(torch.ones_like(self.mask))

    def forward(self, x):
        total_weight_length = self.in_channels*self.out_channels*self.kernel_size*self.kernel_size
        conv_weight_scattered = \
            torch.zeros(total_weight_length).scatter_(0,
                                                      self.mask, self.conv_weight).view(
                                                      (self.kernel_size, self.kernel_size))
        out_normal = F.conv2d(input=x, weight=conv_weight_scattered, bias=self.conv.bias, stride=self.conv.stride,
                              padding=0, dilation=self.conv.dilation, groups=self.conv.groups)
...
class Conv2d_Hori_Veri_Cross(nn.Module):
    ...
    def forward(self, x):
        ...
        tensor_zeros = torch.zeros(C_out, C_in, 1, device=self.conv.weight.device, dtype=self.conv.weight.dtype)
