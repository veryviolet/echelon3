...
class PairTrainer(Trainer):
    ...
    ``net(base, query, <return-features flag>)`` — the flag is passed
    **positionally**, so nets are free to name it (``return_features``,
    ``return_intermediates``, …). It returns a ``heatmap``, or
    ``(heatmap, features)`` when features are requested (the default). Every
    ...
    def compute_losses(self, source, labels, net=None):
        ...
        if self._return_features:
            # Positional flag: nets name it return_features / return_intermediates /
            # … — passing it by position keeps the trainer agnostic to the name.
            heatmap, features = net(base, query, True)
        else:
