...
class MultiPartBatchSampler(BatchSampler):
    ...
    def __init__(self, dataset: MultiPartDataset, batch_size: int, drop_last: bool) -> None:
    ...
    def prepare_idxs(self):
        ...
        max_part_len = self.dataset.part_len(max_part)
        ...
        self.idxs[max_part] = list(range(max_part_len))
    ...
    def __len__(self) -> int:
        max_part = self.dataset.max_part()
        max_part_batch_size = int(self.dataset.part_share(max_part) * self.batch_size)
        ...
        if self.drop_last:
            return self.dataset.part_len(max_part) // max_part_batch_size
        else:
            return (self.dataset.part_len(max_part) + max_part_batch_size - 1) // max_part_batch_size
